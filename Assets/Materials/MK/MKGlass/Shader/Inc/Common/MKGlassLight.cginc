//Light Calculations
	...
	void MKGlassLightMain(inout MKGlassSurface mkts, in VertexOutputForward o)
	{
			...
			c.rgb = lerp(c.rgb * mkts.Pcp.LightColorXAttenuation, c.rgb * lerp(1.0h, mkts.Pcp.NdotL, _DiffuseIntensity) * mkts.Pcp.LightColorXAttenuation, _ShadowIntensity);
	...
	void MKGlassLightFinal(inout MKGlassSurface mkts, in VertexOutputForward o)
	{
					...
					//apply rim lighting as emission
					mkts.Color_Emission += RimDefault(_RimSize, mkts.Pcp.VdotN, _RimColor.rgb, _RimIntensity) * mkts.Pcp.LightColorXAttenuation;
				#endif
